...
Use this section before filling chapter decisions.

- Episode-level or chapter-level changes to source outcomes, casualty counts, protected canon agency, power availability, kill/finalization ownership, legal aftermath, relationship-state implications, or other result-bearing facts must be registered in `work/c2eX/CANON_DELTA.md` before this plan applies them.
...
- Required deaths, injuries, captures, escapes, losses, custody changes or legal conditions:
...
- Obstacles / terrain / objects that affect line of sight or movement:
- Possession state of relevant objects and weapons:
...
- What Mielle must not reveal about herself unless directly pressed:
...
- Meaningful capability, item, inference or defense:
- Relevant source mechanic or setting constraint:
- Capabilities used:
- Capabilities explicitly prohibited:
- Combat pre-check required: yes / no, with reason:
...
- Event tile candidates:
- Knowledge ledger candidates:
- Character-state candidates:
- Relationship-state candidates:
- Open-thread candidates:
- Facts that must not be promoted from plan alone:
