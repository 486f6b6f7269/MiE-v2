...
| Combat, pursuit, physical threat, hostile magic, meaningful gear use, consequential power use, or a material item condition that constrains a later conflict | `canon/COMBAT_RULES.md`; relevant `canon/mielle_current_power_guide/` module(s); completed pre-check from `templates/COMBAT_TRANSLATION_CARD_TEMPLATE.md`; `canon/HOLMES_ENGINE.md` if tactic depends on reading or inference |
...
| Draft | completed loadout and every exact path it names; completed decision-to-authority check; Anti-AI pre-draft guard; `canon/CHAPTER_PROSE_AND_CHARACTER_VOICE_PASS.md`; sequential passes in the workflow: factual/continuity, character/relationship, mechanics/Holmes/combat, prose/voice | draft plus completed verification-pass receipt |
